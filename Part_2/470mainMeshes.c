...
dimensions 3 (xyz), 2 (st), 3 (nop). */
#define MESHNUMATTRS 3
int meshAttrDims[MESHNUMATTRS] = {3, 2, 3};
...
veshVesh veshBox, veshBox1;


int initializeArtwork() {
    meshMesh meshBox, meshBox1;

    if (shaInitialize(&shaProg, "470vert.spv", "460frag.spv") != 0) {
        return 6;
    }
    if (mesh3DInitializeBox(&meshBox, -5.0, -3.0, 3.0, 4.0, -2.0, -1.0) != 0){
        shaFinalize(&shaProg);
        return 5;
    }
    if (mesh3DInitializeBox(&meshBox1, -7.0, -1.0, -0.5, 0.0, -3.0, -2.5) != 0){
        meshFinalize(&meshBox);
        shaFinalize(&shaProg);
        return 4;
    }

    if (veshInitializeStyle(&veshSty, MESHNUMATTRS, meshAttrDims) != 0){
        meshFinalize(&meshBox1);
        meshFinalize(&meshBox);
        shaFinalize(&shaProg);
        return 3;
    }
    if (veshInitializeMesh(&veshBox, &meshBox)!=0){
        veshFinalizeStyle(&veshSty);
        meshFinalize(&meshBox1);
        meshFinalize(&meshBox);
        shaFinalize(&shaProg);
        return 2;
    }
    if(veshInitializeMesh(&veshBox1, &meshBox1)!=0){
    ...
    // we don't need meshMesh after making veshVesh
    meshFinalize(&meshBox1); 
    meshFinalize(&meshBox);
...
int initializeCommandBuffers() {
    ...
    for (size_t i = 0; i < swap.numImages; i += 1) {
        ...
        /* Render a mesh. */
        veshRender(&veshBox1, connCommandBuffers[i]); 
...
int initializeConnection() {
    if (initializePipeline(&shaProg, &veshSty.vertexInputInfo, &veshSty.inputAssembly) != 0)
        return 2;
