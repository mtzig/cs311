    ...
    int numVerts, const float verts[])
{
    ...
    if (bufInitialize(
            bufSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
    ...
    const uint16_t tris[])
{
    ...
    /* Create a GPU buffer, from which to actually render. */
    if (bufInitialize(
            bufSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
...
int veshInitializeMesh(veshVesh *vesh, meshMesh *mesh)
{
    ...
    if (veshInitializeIndexBuffer(
            &vesh->triBuf, &vesh->triBufMem, vesh->triNum, mesh->tri) != 0)
    {
