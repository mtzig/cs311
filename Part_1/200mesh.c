...
void meshRender(
	...
	for(int tri = 0; tri < mesh->triNum; tri++){
		...
		shadeVertex(sha->unifDim, unif, sha->attrDim, meshGetVertexPointer(mesh, verts[0]), sha->varyDim, a);
		shadeVertex(sha->unifDim, unif, sha->attrDim, meshGetVertexPointer(mesh, verts[1]), sha->varyDim, b);
		shadeVertex(sha->unifDim, unif, sha->attrDim, meshGetVertexPointer(mesh, verts[2]), sha->varyDim, c);
