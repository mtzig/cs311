...
void pixSetInterpolate(double x0, double x1, const double a[2], const double A_I[2][2], 
                       const double alpha[3], const texTexture *tex, const double betaMalpha[2], 
                       const double gammaMalpha[2], const double rgb[2]){
            ...
            double st_coords[2]; //vector for interpolated

 

            double p_ba[2], q_ga[2];
            vecScale(2, pq[0], betaMalpha, p_ba);
            vecScale(2, pq[1], gammaMalpha, q_ga);

            // interpolation
            vecAdd(2, p_ba, q_ga, st_coords);
            vecAdd(2, st_coords, alpha, st_coords);
...
void triRenderWithAleft(
    ...
    double betaMalpha[2], gammaMalpha[2];
    ...
    vecSubtract(2, beta, alpha, betaMalpha);
    vecSubtract(2, gamma, alpha, gammaMalpha);
...
void triRender(
    ...
    if (a[0] <= b[0] && a[0] <= c[0])
        ...
        triRenderWithAleft(b, c, a, rgb, tex, beta, gamma, alpha);
    else
        triRenderWithAleft(c, a, b, rgb, tex, gamma, alpha, beta);
