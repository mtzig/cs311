    ...
    const double c[])
{
    ...
    { // c is between a,b
        ...
        if (a[VARYX] != c[VARYX])
        { // render left half
            for (int x0 = MAX((int)ceil(a[VARYX]), 0); x0 <= floor(c[VARYX]) && x0 <= buf->width; x0++)
            {
        ...
        if (b[VARYX] != c[VARYX])
        { // render right half
            for (int x0 = MAX((int)floor(c[VARYX]) + 1, 0); x0 <= floor(b[VARYX]) && x0 <= buf->width; x0++)
            {
                int bottom = (int)ceil(pointSlope(x0, a, b));
        ...
        if (a[VARYX] != b[VARYX])
        { // render left half
            for (int x0 = MAX((int)ceil(a[VARYX]), 0); x0 <= floor(b[VARYX]) && x0 <= buf->width; x0++)
            {
                int bottom = (int)ceil(pointSlope(x0, a, b));
                int top = (int)floor(pointSlope(x0, a, c));
    ...
    if (b[VARYX] != c[VARYX])
    { // render right half
        for (int x0 = MAX((int)floor(b[VARYX]) + 1, 0); x0 <= floor(c[VARYX]); x0++)
        {
