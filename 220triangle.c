...
void triRenderWithAleft(
    ...
    double bMa[varyDim], cMa[varyDim];
    ...
    if (a[VARYX] <=c[VARYX] && c[VARYX] <= b[VARYX]){ // c is between a,b

        if (a[VARYX] != c[VARYX]){ //render left half
            for (int x0 = (int) ceil(a[VARYX]); x0 <= floor(c[VARYX]); x0++){
                ...
                for (int x1 = bottom; x1 <= top; x1++){
                    ...
                    sha->shadeFragment(sha->unifDim, unif, sha->texNum, tex, varyDim, vary, rgb);
        ...
        if (b[VARYX] != c[VARYX]){ //render right half
        ...
        if (a[VARYX] != b[VARYX]){ //render left half
