...
int initializeArtwork(void) {
    ...
    if (bodyInitialize(&bodies[5], GEOMUNIFDIM, MATERUNIFDIM, TEXNUM, 
                       &reshGetIntersection, &reshGetTexCoordsAndNormal, 
                       &getPhongMaterial)!= 0){
    ...
    // really crappy capsule
    if (mesh3DInitializeCapsule(&mesh, 0.5, 1.0, 3, 3) != 0){
        lightFinalize(&lights[LIGHTPOS]);
        lightFinalize(&lights[LIGHTDIR]);
        bodyFinalize(&bodies[0]);
        bodyFinalize(&bodies[1]);
